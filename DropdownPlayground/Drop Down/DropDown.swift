...
// TODO: Rename to something not stupid
protocol DropDownInterface: AnyObject {
    func cancel()
    var isHidden: Bool { get set }
}

struct DropDownGlobals {
    /// The current visible drop down. There can be only one visible drop down at a time.
    public static weak var VisibleDropDown: DropDownInterface?
...
public final class DropDown<OptionType>: UIView,
    DropDownInterface,
    UITableViewDataSource,
UITableViewDelegate {

    public typealias SelectionClosure = (Index, OptionType) -> Void
    public typealias MultiSelectionClosure = ([Index], [OptionType]) -> Void
    public typealias ConfigurationClosure = (Index, OptionType) -> String
    public typealias CellConfigurationClosure = (Index, OptionType, DropDownCell) -> Void
    ...
    @objc public dynamic var headerHeight = DPDConstant.UI.HeaderHeight {
        didSet { reloadAllComponents() }
    }
    ...
    /**
     ...
     Changing the text color automatically reloads the drop down.
     */
    @objc public dynamic var headerTitleTextColor = DPDConstant.UI.HeaderTitleTextColor {
        didSet { reloadAllComponents() }
    }
    ...
    /**
     ...
     Changing the text font automatically reloads the drop down.
     */
    @objc public dynamic var headerTitleTextFont = DPDConstant.UI.HeaderTitleTextFont {
        didSet { reloadAllComponents() }
    }
    ...
    /**
     ...
     Changing the text font automatically reloads the drop down.
     */
    @objc public dynamic var headerTitleBackGroundColor = DPDConstant.UI.HeaderTitleBackgroundColor {
        didSet { reloadAllComponents() }
    }
     ...
     */
    public var dataSource = [OptionType]() {
        didSet {
    ...
    // Not used, and keeping this will make conversion to generic more difficult
    /*
     ...
     */
    public convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, dataSource: [OptionType] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil) {
        self.init(frame: .zero)
    ...
    public func show(beforeTransform transform: CGAffineTransform? = nil, anchorPoint: CGPoint? = nil) -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
        if self === DropDownGlobals.VisibleDropDown && DropDownGlobals.VisibleDropDown?.isHidden == false { // added condition - DropDown.VisibleDropDown?.isHidden == false -> to resolve forever hiding dropdown issue when continuous taping on button - Kartik Patel - 2016-12-29
            return (true, 0)
        }

        if let visibleDropDown = DropDownGlobals.VisibleDropDown {
            visibleDropDown.cancel()
        }

        willShowAction?()

        DropDownGlobals.VisibleDropDown = self
        ...
        UIView.animate(
            ...
            delay: 0,
            options: DropDownGlobals.animationEntranceOptions,
    ...
    /// Hides the drop down.
    public func hide() {
        if self === DropDownGlobals.VisibleDropDown {
            /*
             ...
             */
            DropDownGlobals.VisibleDropDown = nil
        }
        ...
        UIView.animate(
            withDuration: animationduration,
            delay: 0,
            options: DropDownGlobals.animationExitOptions,
            animations: { [weak self] in
    ...
    public var selectedItem: OptionType? {
    ...
    fileprivate var tableHeight: CGFloat {
        return tableView.rowHeight
            * CGFloat(dataSource.count)
            + (self.title == "" ? 0 : headerHeight)
    }
    ...
    public func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        label.textColor = UIColor(red: 153 / 255,
                                  green: 153 / 255,
                                  blue: 153 / 255,
                                  alpha: 1.0)
        return label
    ...
    fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
        ...
        if let cellConfiguration = cellConfiguration {
            ...
            cell.optionLabel.text = "DEV: Must Provide Cell Configuration" // TODO: enforce with compiler
        }
